...
class PresentationDisplaysPlugin : FlutterPlugin, ActivityAware, MethodChannel.MethodCallHandler {
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        ...
        when (call.method) {
            "showPresentation" -> {
                try {
                    val displayId: Int = call.argument<Int>("displayId") as Int
                    val args: String = call.argument<String>("arguments") as String
                    Log.i(
                        TAG,
                        "Channel: method: ${call.method} | displayId: $displayId"
                    )

                    val display = displayManager?.getDisplay(displayId)
                    if (display != null) {
                        val flutterEngine = createFlutterEngine(displayId.toString() ,args  )
                        flutterEngine?.let {
                            ...
                            val presentation =
                                context?.let { it1 -> PresentationDisplay(it1,  displayId.toString() ,    display) }
    ...
    private fun createFlutterEngine(tag: String ,  args: String ): FlutterEngine? {
        if (context == null)
            return null
        if (FlutterEngineCache.getInstance().get(tag) == null) {
            val list = ArrayList<String>()
            list.add( 0 , "multi_tablet")
            list.add( 1 , args)
            val flutterEngine = FlutterEngine(context!! , list.toTypedArray() )
